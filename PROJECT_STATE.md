...
- **P20 Evidence:** the Owner rejected the prior interface before testing and explicitly authorized a JustFair-structured Dashboard plus App restructure while retaining CLINCH identity and product truth. The correction pass now includes live Bitget Reality instrument discovery, package-backed verified company marks with truthful fallbacks, fresh-session Research Again semantics, and one canonical responsive stock-selection flow for Featured/Browse/search cards. The final stock-selection correction is deployed at `https://clinch-nine.vercel.app` from `7ccce6c`; public stock discovery returned 1,173 supported Reality instruments, 16 verified marks, and 1,157 fallbacks. Public Featured, Browse, search, replacement, selected-context, desktop, mobile, keyboard, bounded-list, and no-auto-research checks passed. Owner review is now required; P21 remains untouched. Evidence: `P20_UX_RESCUE_PLAN.md`, `BRAND_ASSET_ATTRIBUTION.md`, `components/stock-discovery.tsx`, and `proof/p20-stock-selection/`.
