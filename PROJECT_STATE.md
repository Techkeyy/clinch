...
- **P20 Evidence:** the Owner rejected the prior interface before testing and explicitly authorized a JustFair-structured Dashboard plus App restructure while retaining CLINCH identity and product truth. The correction pass now includes live Bitget Reality instrument discovery, package-backed verified company marks with truthful fallbacks, fresh-session Research Again semantics, one canonical responsive stock-selection flow for Featured/Browse/search cards, and the final optional stock-first App order. The latest order correction is ready for deployment from the current P20 worktree; prior public stock discovery returned 1,173 supported Reality instruments, 16 verified marks, and 1,157 fallbacks. Twelve focused desktop/mobile browser checks passed, covering optional stock context, selected and natural-language-only submission, DOM order, search, replacement, keyboard access, bounded lists, and no-auto-research. Owner review is required; P21 remains untouched. Evidence: `P20_UX_RESCUE_PLAN.md`, `BRAND_ASSET_ATTRIBUTION.md`, `components/stock-discovery.tsx`, and `proof/p20-order-correction/`.
