...
- **P20 Evidence:** the Owner rejected the prior interface before testing and explicitly authorized a JustFair-structured Dashboard plus App restructure while retaining CLINCH identity and product truth. The correction pass now includes live Bitget Reality instrument discovery, package-backed verified company marks with truthful fallbacks, fresh-session Research Again semantics, one canonical responsive stock-selection flow for Featured/Browse/search cards, and the final optional stock-first App order. The latest order correction is deployed from `60e17bc` at `https://clinch-nine.vercel.app` as Vercel deployment `dpl_B8RunHpETgh2RoPqgBydvEeCNGnJ`, READY. Prior public stock discovery returned 1,173 supported Reality instruments, 16 verified marks, and 1,157 fallbacks. Twelve focused local desktop/mobile browser checks plus two fresh public HTTPS desktop/mobile smoke checks passed, covering optional stock context, selected and natural-language-only submission, DOM order, search, replacement, keyboard access, bounded lists, and no-auto-research. Owner review is required; P21 remains untouched. Evidence: `P20_UX_RESCUE_PLAN.md`, `BRAND_ASSET_ATTRIBUTION.md`, `components/stock-discovery.tsx`, and `proof/p20-order-correction/`.
...
- **P20 Research Semantics Correction:** Owner UAT found that wait-before-entry wording could suppress the capable Reality structure family and fall into an unresolved no-capability result. The deterministic compiler now treats wait as entry timing for Reality move and structure questions; capability data follows resolved instruments; asset identity and explicit STOPPED/UNRESOLVED terminal contracts persist across start, continue, retry, and refresh; user copy centrally removes kernel jargon and never shows a placeholder Hinge as final output. The frozen NVDA scenario, true no-capable-family case, STOP contract, copy audit, and fixture browser proof pass. Final public proof against https://clinch-nine.vercel.app on Vercel deployment dpl_x9G5EpGAZk9VoTfxFZXrv7GpY1HR resolved NVIDIA to NVDA / RNVDAUSDT / NVDAUSDT, selected spot-structure, produced a real finding, deliberately stopped, and returned Considering whether to wait before entering NVDA. Evidence: proof/p20-research-semantics/public-nvidia.json and proof/p20-research-semantics/public-nvidia.png. P21 remains untouched; Owner UAT is the next human action.
