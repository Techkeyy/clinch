...
| **P10** | **Bitget Data Layer** | COMPONENT PROVEN | Implement only the real Bitget data capabilities proven necessary by P2-P5 and verified in P4. | Data layer fetches, validates, and normalizes live Bitget data with graceful error fallbacks. Every Bitget integration is load-bearing; no indiscriminate endpoint consumption. | P10 COMPONENT PROVEN: live discovery (1173 Reality + 321 RWA perps), RNVDAUSDT ticker/candles/orderbook, NVDAUSDT positioning, typed failures, 26 unit + 4 live tests pass. |
| **P11** | **Research Layer** | BUILDING | Implement research executors that query specific Bitget capabilities targeted to candidate decision hinges. | Targeted research modules execute against live endpoints and produce structured evidence. | Pending P10 completion. |
