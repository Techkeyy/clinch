...
- **Current Active Phase:** P6 Architecture Lock
...
| **P6** | **Architecture Lock** | BUILDING | Formulate technical pipeline, module boundaries, data flow, Decision Hinge architecture, and error boundaries. | Architecture document approved with strict module-by-job separation and offline fixture strategy. Architecture locked only after P5 evidence exists. | P6 evidence: P6_ARCHITECTURE_LOCK.md (Next.js single deployable + Neon/Drizzle + Qwen-compatible model interface + direct REST adapter; P6 BUILDING pending Director). |
